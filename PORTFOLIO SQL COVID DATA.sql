...
select continent, Max(total_cases) as "Total Number of Cases", max((total_cases/population)) * 100 as "Percentage of Population that has had Covid"
from PortfolioProject..CovidDeaths
...
select continent, Max(total_deaths) as "Total Deaths", max((total_deaths/population)) * 100 as "Percentage of Continent that has died with Covid"
from PortfolioProject..CovidDeaths
where continent is not null
group by continent
order by 3 desc
...
select date, sum(total_cases) as "Total Cases", sum(cast(total_deaths as int)) as "Total Deaths", sum(cast(total_deaths as int))/sum(total_cases)*100 as "Global Death Rate"
from PortfolioProject..CovidDeaths
group by date
order by 1
